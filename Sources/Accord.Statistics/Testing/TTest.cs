...
///   through its <see cref="Analysis"/> property.</para>
///   
/// <para>
///   References:
///   <list type="bullet">
...
public class TTest : HypothesisTest<TDistribution>
{
    ...
    protected void Compute(int n, double mean, double hypothesizedMean, double stdError, OneSampleHypothesis hypothesis)
    {
        ...
        HypothesizedValue = hypothesizedMean;
    ...
    protected void Compute(double statistic, double df, OneSampleHypothesis alternate)
    {
        ...
        Tail = (DistributionTail)alternate;
    ...
    {
        powerAnalysis = new TTestPowerAnalysis(Hypothesis)
        {
            Samples = samples,
    ...
    public static double StatisticToPValue(double t, TDistribution distribution, DistributionTail type)
    {
        var p = type switch
        {
            DistributionTail.TwoTail => 2.0 * distribution.ComplementaryDistributionFunction(Math.Abs(t)),
            DistributionTail.OneUpper => distribution.ComplementaryDistributionFunction(t),
            DistributionTail.OneLower => distribution.DistributionFunction(t),
    ...
    public static double PValueToStatistic(double p, TDistribution distribution, DistributionTail type)
    {
        var t = type switch
        {
            DistributionTail.OneLower => distribution.InverseDistributionFunction(p),
            DistributionTail.OneUpper => distribution.InverseDistributionFunction(1.0 - p),
            DistributionTail.TwoTail => distribution.InverseDistributionFunction(1.0 - p / 2.0),
            _ => throw new InvalidOperationException(),
        };
        return t;
    }
