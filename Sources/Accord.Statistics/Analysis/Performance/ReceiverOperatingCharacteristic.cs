...
global using Accord.Statistics.Visualizations;
global using Accord.Statistics.Testing;
...
public class ReceiverOperatingCharacteristic
{
    ...
    public void Compute(double increment, bool forceOrigin)
    {
        ...
        collection = new ReceiverOperatingCharacteristicPointCollection(points.ToArray());

        // Calculate area and error associated with this curve
        area = CalculateAreaUnderCurve();
        error = CalculateStandardError();
    }
    ...
    public void Compute(params double[] cutpoints)
    {
        if (cutpoints.Length == 0) throw new ArgumentException(
            "Cutpoints array must have at least one value.", nameof(cutpoints));

        List<ReceiverOperatingCharacteristicPoint> points = new();
    ...
    public ReceiverOperatingCharacteristicPoint ComputePoint(double threshold)
    {
        ...
        {
            bool actual = measurement[i] == dtrue;
            bool predicted = prediction[i] >= threshold;
    ...
    /// <summary>
    ///   Returns a <see cref="string"/> that represents this curve.
    /// </summary>
    /// <returns>
    ///   A <see cref="string"/> that represents this curve.
    /// </returns>
    public override string ToString()
    {
        ...
        return string.Format(System.Globalization.CultureInfo.CurrentCulture,
            "AUC:{0:0.0} Error:{1:0.0} Positives:{2} Negatives:{3}",
             Area, StandardError, Positives, Negatives);
    }
    #endregion
    ...
    private double CalculateAreaUnderCurve()
    {
        ...
        for (int i = 0; i < collection.Count - 1; i++)
        {
            ...
            double tpz = collection[i].Sensitivity + collection[i + 1].Sensitivity;
    ...
    {
        return System.Math.Sqrt(ReceiverOperatingCurveTest.DeLongVariance(positiveAccuracy, negativeAccuracy));
    }

    private void CalculatePlacements()
    {
        // Get ratings for true positives
        int[] positiveIndices = measurement.Find(x => x == dtrue);
        double[] X = prediction.Get(positiveIndices);

        int[] negativeIndices = measurement.Find(x => x == dfalse);
        double[] Y = prediction.Get(negativeIndices);
    ...
    /// 
    public static async Task<ReceiverOperatingCharacteristic> LoadAsync(Stream stream)
    {
        return (ReceiverOperatingCharacteristic)await JsonSerializer.DeserializeAsync(stream, typeof(ReceiverOperatingCharacteristic)).ConfigureAwait(false);
    }
    ...
    /// <returns>The deserialized curve.</returns>
    /// 
    public static async Task<ReceiverOperatingCharacteristic> LoadAsync(string path)
    {
